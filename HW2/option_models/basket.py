...
def basket_price_mc(
    ...
    if (bsm):
        ...
        '''
        prices = forward[:, None] * np.exp(np.sqrt(texp) * chol_m @ znorm_m - texp * np.power(vol[:, None], 2)/2)
    else:
...
def basket_price_norm_analytic(
    ...
    disc_fac = np.exp(-texp * intr)
    forward = spot / disc_fac * div_fac
    spot = forward @ weights[:,None] * disc_fac
    cov_m = vol * cor_m * vol[:,None]
    vol_norm = np.sqrt(weights @ cov_m @ weights.T)
    price = normal_formula(strike, spot, vol_norm, texp, intr=intr, divr=divr, cp_sign=cp_sign)
    return price[0]
